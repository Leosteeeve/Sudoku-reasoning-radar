...
test("OCR staging fails clearly for a stub helper or missing eng data", async () => {
  ...
  try {
    await assert.rejects(() => stageOcrRuntime({ helperPath: path.join(stub.build, "srr-ocr-helper.exe"), tessdataDir: stub.tessdata, outputDir: stub.output }), /without OCR support/i);
  } finally { await rm(stub.root, { recursive: true, force: true }); }

  const missing = await fixture();
  try {
    await rm(path.join(missing.tessdata, "eng.traineddata"));
    await assert.rejects(() => stageOcrRuntime({ helperPath: path.join(missing.build, "srr-ocr-helper.exe"), tessdataDir: missing.tessdata, outputDir: missing.output }), /eng\.traineddata/i);
