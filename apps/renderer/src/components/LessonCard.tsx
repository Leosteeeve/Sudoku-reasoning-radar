...
import { localizedAction, techniqueNames, translate } from "../i18n";
...
export function LessonCard({ step, language }: { step?: SolveStepV1; language: Language }) {
  ...
  const elimination = digits
    ? translate(language, "eliminateBody", { digits })
    : translate(language, "noEliminationBody");
  const conclusion = step ? localizedAction(language, step.action) : translate(language, "noStep");
  return (
    <article className="lesson-card" data-testid="lesson-card" aria-label={translate(language, "lesson")}>
      ...
      <ol className="lesson-sequence">
        ...
        <li>
          ...
          <div><h3>{translate(language, "eliminate")}</h3><p>{elimination}</p></div>
        </li>
        <li>
          <span className="lesson-number">03</span>
          <div><h3>{translate(language, "conclude")}</h3><p>{translate(language, "concludeBody", { action: conclusion })}</p></div>
        </li>
      </ol>
    </article>
  );
